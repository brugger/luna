 ...
 *****************************************************************************/
  ...
  // Skaber en matrice der er hlen * vlen stor. 
  matrix->matrix = (int **) malloc((matrix->hlen +1)* sizeof(int*));
  ...
  for (i = 0; i < matrix->hlen; i++){
    matrix->matrix[i] = (int*) malloc((matrix->vlen+1) * sizeof(int));
  ...
  for (i = 0; i < matrix->hlen-1; i++) 
    for (j = 0; j < matrix->vlen-1; j++) 
      matrix->matrix[i][j] = 0;
  ...
  for(i = matrix->hlen - 1; i >= 0;i--) {
	...
	(matrix->matrix[i][j] == matrix->matrix[i-1][j] + GapValue)) {
      ...
      k--;
	     ...
	     (matrix->matrix[i][j] == matrix->matrix[i-1][j-1]) + p(vseq[j],hseq[i])) {
      s1[k] = vseq[j];
      s2[k] = hseq[i];
      k--;
      j--; 
